...
#   Retrained graph
MODEL_PATH = os.path.join(sys.path[0], 'retrain_graph/output_graph.pb')
#   Labels the newly retrained graph.  These would be the new classes being classified 
#       such as "Rose, Dandillion, ..."
LABEL_PATH = os.path.join(sys.path[0], 'retrain_graph/output_labels.txt')
...
def predict_image_class(imagePath, labelPath, modelPath, har):
    ...
    # crop the input with har
    if hasattr(args, 'har'):
        face_cascade = cv2.CascadeClassifier(args.har)
        if face_cascade == "False":
            raise ValueError('cascade directory is not valid')
        frame = cv2.imread(imagePath)
        faces = face_cascade.detectMultiScale(frame, 1.3, 5)
        (x, y, w, h) = faces[0]
        roi = frame[y:y + h, x:x + w]
        cv2.imwrite(imagePath+'cropped.jpg', roi)
    # Load the image from file
        image_data = tf.gfile.FastGFile(imagePath + 'cropped.jpg', 'rb').read()
    else:
    ...
    with tf.Session() as sess:
        ...
        try:
            os.remove(imagePath + 'cropped.jpg')
        except OSError:
            pass
...
if __name__ == '__main__':
    ...
    parser.add_argument(
      ...
      type=str,
      default='False',
      help='(optional) Path of har-classifier to crop input image.'
    )
    ...
    if args.image_path.lower().endswith(('.jpg', '.jpeg')):
        ...
        predict_image_class(args.image_path, LABEL_PATH, MODEL_PATH, args.har)
    else:
